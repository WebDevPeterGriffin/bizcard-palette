-- Allow public updates to headshot_url while protecting other fields
-- 1) Create an UPDATE policy for anon/authenticated roles
CREATE POLICY IF NOT EXISTS "Anyone can update headshot"
ON public.cards
FOR UPDATE
...
-- 2) Create a trigger function to restrict which columns can be updated publicly
...
AS $$
BEGIN
  -- Disallow changes to any columns except headshot_url (and updated_at which is auto-managed)
  IF NEW.id <> OLD.id
     ...
     OR COALESCE(NEW.headshot_url, '') = COALESCE(OLD.headshot_url, '') -- ensure actual change happens
  THEN
...
-- 3) Attach the trigger to the cards table
...
-- 4) Ensure updated_at auto-updates on any update
